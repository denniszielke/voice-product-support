...
DESCRIPTION:
    VoiceLive client for the CyclePro Bike Renting agent (and any other
...
# ---------------------------------------------------------------------------
# UI event rendering (custom ui.* events from the bike-renting agent)
# ---------------------------------------------------------------------------


def _print_ui_event(evt: dict[str, Any]) -> None:
    etype = evt.get("type", "event")
    if etype == "ui.bike_cards":
        print("  [Bikes shown]")
        for b in evt.get("bikes", []):
            price = b.get("rental_price_per_day_eur")
            price_str = f" — {price:.0f} EUR/day" if isinstance(price, (int, float)) else ""
            print(f"    - {b.get('name', '?')}{price_str} (id={b.get('id')})")
    elif etype == "ui.bike_detail":
        b = evt.get("bike", {})
        print(
            f"  [Selected] {b.get('name')} — {b.get('rental_days')} day(s), "
            f"total {b.get('total_eur')} EUR + {b.get('deposit_eur')} deposit"
        )
    elif etype == "ui.action_buttons":
        labels = [a.get("label") for a in evt.get("actions", [])]
        print(f"  [Actions] {', '.join(filter(None, labels))}")
    elif etype == "ui.reservation_confirmed":
        r = evt.get("reservation", {})
        print(f"  [Reserved] {r.get('bike_name')} — code {r.get('reservation_code')}")
    elif etype == "ui.booking_confirmed":
        b = evt.get("booking", {})
        print(
            f"  [Booked] {b.get('bike_name')} — code {b.get('confirmation_code')}, "
            f"total {b.get('total_eur')} EUR"
        )
    elif etype == "ui.booking_update":
        print(f"  [Update] {json.dumps({k: v for k, v in evt.items() if k != 'type'})}")
    else:
        print(f"  [{etype}] {json.dumps({k: v for k, v in evt.items() if k != 'type'})}")
...
class VoiceLiveClient:
    ...
    * If ``invocation_url`` is provided instead, VoiceLive runs as a
    ...
    def _print_banner(self) -> None:
        print("\n" + "=" * 60)
        print("  CYCLEPRO BIKE RENTING — VOICE CLIENT (VoiceLive)")
        if self.agent_config is not None:
        ...
        print("  Try: 'I'd like to rent a mountain bike for three days'")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "VoiceLive client for the CyclePro bike-renting agent. "
    ...
    parser.add_argument(
        "--agent-name",
        default=os.getenv("AZURE_AI_HOSTED_AGENT_NAME", "bike-renting"),
        help="Hosted-agent mode: the deployed invocations agent name "
             "(default: bike-renting).",
    )
    parser.add_argument(
        "--project-name",
    ...
    print("  CyclePro Bike Renting — VoiceLive Client")
    print("=" * 60)
