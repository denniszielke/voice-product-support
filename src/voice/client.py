...
DESCRIPTION:
    Voice client for the CyclePro Bike Support agent using Azure VoiceLive SDK.
    Connects to the prompt-based bike-concierge agent deployed in Azure AI Foundry.
...
USAGE:
    ...
    Command-line arguments:
      ...
      --agent-name     The name of the deployed agent (default: bike-concierge)
      --project-name   The Foundry project containing the agent (required)
...
try:
    ...
    print("This client requires pyaudio. Install with:")
...
class VoiceProductGuideClient:
    ...
    async def _handle_event(self, event):
        ...
        if event.type == ServerEventType.SESSION_UPDATED:
            ...
            print(f'\n  You: {event.get("transcript", "")}')

        elif event.type == ServerEventType.RESPONSE_TEXT_DONE:
            pass

        elif event.type == ServerEventType.RESPONSE_AUDIO_TRANSCRIPT_DONE:
            print(f'\n  Agent: {event.get("transcript", "")}')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CyclePro Voice Product Guide - Azure VoiceLive SDK client"
    )
    parser.add_argument(
        "--endpoint",
        default=os.getenv("AZURE_VOICELIVE_ENDPOINT", ""),
        help="The Azure VoiceLive endpoint (or set AZURE_VOICELIVE_ENDPOINT)",
    )
    parser.add_argument(
        "--agent-name",
    ...
    parser.add_argument(
        ...
        help="The Foundry project containing the agent (or set AZURE_AI_PROJECT_NAME)",
    )
    ...
    if not args.project_name:
        print("Error: --project-name is required (or set AZURE_AI_PROJECT_NAME)")
        sys.exit(1)
    ...
    try:
        ...
        if not input_devices:
            print("Error: No audio input devices found. Please check your microphone.")
            sys.exit(1)
        if not output_devices:
            print("Error: No audio output devices found. Please check your speakers.")
            sys.exit(1)
    except Exception as e:
